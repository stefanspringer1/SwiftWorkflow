...
public struct LoggingEvent: CustomStringConvertible, Encodable, Sendable {
    ...
    public var description: String {
        return "\(self.type > .Error ? "{\(MessageType.Error)}" : ""){\(self.type)}\(self.originalType != nil ? " <- \(self.originalType!)" : "")\(self.messageID != nil ? " [\(self.messageID ?? "")]": ""): \(fact[.en]?.trimming() ?? "?")\(solution != nil ? " – solution: \(solution?[.en]?.trimming() ?? "?")" : "")" + (self.itemPositionInfo != nil ? " @ \(self.itemPositionInfo!)" : "")
    }
